    ...
    const handleSubmit = async () => {
        try {
            ...
            if (error.response?.status === 404) {
                alert("vendor already exists. Please Signin");
            } else {
                alert("Internal server error during signup");
            }
            console.log("error in vendor signup:", error.response?.data || error.message);
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="max-w-md mx-auto">
                <div className="bg-white rounded-2xl shadow-xl p-8">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-gray-600 mt-2">Create new vendor account</p>
                    </div>
                    {submitSuccess && (
                        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                            <p className="text-green-800 text-center font-medium">✓ vendor Account created successfully!</p>
                        </div>
                    )}
                    {step === 1 && (
