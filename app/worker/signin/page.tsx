   ...
   const handleSubmit = async (e: any) => {
    ...
    // Only proceed with API call if validation passes
     try {
            ...
            const response = await axios.post('/api/worker/signin', formData)
            ...
            if (error.response?.status === 400) {
                ...
                alert("No worker found. Try signing up first.");
            } else {
                alert("Internal server error during signin");
            }
            console.log("error in worker signin:", error.response?.data || error.message);
            ...
            <div className="max-w-md mx-auto">
                <div className="bg-white rounded-2xl shadow-xl p-8">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-gray-600 mt-2">Sign in to worker account</p>
                    </div>

                    {submitSuccess && (
                        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                            <p className="text-green-800 text-center font-medium">✓ Worker logged in successfully!</p>
                        </div>
                    )}
